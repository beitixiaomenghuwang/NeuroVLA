...
Write a configuration and register a `robot_config` with `ROBOT_TYPE_CONFIG_MAP` in `NeuroVLA/dataloader/gr00t_lerobot/data_config.py` based on your data keys and normalization methods.

### Step 1: Associate Robot Config with Embodiment Tag
In `NeuroVLA/dataloader/gr00t_lerobot/embodiment_tags.py`, associate the `robot_config` with `EmbodimentTag` to enable merging statistics of the same type of embodiment.

### Step 2: Define Dataset Mixtures
Define the dataset collections to be used in `NeuroVLA/dataloader/gr00t_lerobot/mixtures.py`. For example, define a mixture using two datasets:
...
### Step 3: Data Loading
Finally, you can test data loading by running `NeuroVLA/dataloader/lerobot_datasets.py` through configuration.
...
Configure training-related parameters in `NeuroVLA/config/training/qwenvla_cotrain_custom.yaml`, and modify `data_mix` to your previously configured training recipe:
...
accelerate launch --config_file NeuroVLA/config/deepseeds/deepspeed_zero2.yaml   --num_processes 8 NeuroVLA/training/train_internvla.py --config_yaml NeuroVLA/config/training/internvla_cotrain_custom.yaml

# Fine-tune the model with your action and vision-language data
accelerate launch --config_file NeuroVLA/config/deepseeds/deepspeed_zero2.yaml   --num_processes 8 NeuroVLA/training/train_internvla_cotrain.py --config_yaml NeuroVLA/config/training/internvla_cotrain_custom.yaml
```
...
```
accelerate launch --config_file NeuroVLA/config/deepseeds/deepspeed_zero2.yaml   --num_processes 8 NeuroVLA/training/train_internvla.py   --config_yaml NeuroVLA/config/training/internvla_cotrain_sim_demo.yaml 
```
